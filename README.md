# DB Curator implementation

The active implementation contract is [001-curate-jira-kedb](specs/001-curate-jira-kedb/spec.md).
The code currently provides the package boundaries, validated settings, application composition,
health endpoints, initial pure domain rules, application port declarations, explicit checkpoint setup,
reproducible mock incidents, and approved-current exact/lexical/vector retrieval with persisted rank fusion.
Curation, review, publication, and chat are tracked in
[tasks.md](specs/001-curate-jira-kedb/tasks.md); the design below is not a claim of implemented behavior.
...
$env:DB_CURATOR_DATABASE_URL = 'postgresql://db_curator:db_curator@localhost:5432/db_curator'
uv run uvicorn db_curator.bootstrap.container:create_app --factory --host 127.0.0.1
```

Settings are read from the process environment, never automatically from `.env`. The checkpoint
URL defaults to the business URL. Initialize business tables with `uv run alembic upgrade head`
and checkpoints with `uv run python -m db_curator.bootstrap.checkpoint_setup`. The API exposes
`/api/v1/health/live` and `/api/v1/health/ready`. Readiness probes PostgreSQL, the checkpoint
connection, Chroma, and the configured inference provider; an unavailable dependency returns 503.
It currently checks connectivity, not migration/model/index readiness.

| Environment variable | Default |
|---|---|
| `DB_CURATOR_DATABASE_URL` | Required PostgreSQL URL |
| `DB_CURATOR_CHECKPOINT_DATABASE_URL` | Business URL |
| `DB_CURATOR_LLM_PROVIDER` | `ollama` |
| `DB_CURATOR_OLLAMA_BASE_URL` | `http://localhost:11434` |
| `DB_CURATOR_CHROMA_BASE_URL` | `http://localhost:8001` |
| `DB_CURATOR_GENERATION_MODEL` | `qwen3:8b` |
| `DB_CURATOR_EMBEDDING_MODEL` | `qwen3-embedding:0.6b` |
| `DB_CURATOR_GROQ_API_KEY`, `DB_CURATOR_GROQ_MODEL` | Required only when explicitly selecting Groq |
| `DB_CURATOR_PROVIDER_TIMEOUT_SECONDS` | 60, maximum 120 |
| `DB_CURATOR_SQL_TIMEOUT_SECONDS` | 5, maximum 5 |
| `DB_CURATOR_SEARCH_TIMEOUT_SECONDS` | 10, maximum 10 |
| `DB_CURATOR_EMBEDDING_TIMEOUT_SECONDS` | 30, maximum 30 |
| `DB_CURATOR_MAX_SOURCE_CHARACTERS` | 50000, maximum 50000 |

Compose binds PostgreSQL and Chroma to loopback and preserves their data in named volumes.
The included database credentials are for this local development stack. Use a Compose override
if a local service or Windows reservation occupies the default ports.
...
For disposable service checks, explicitly set `TEST_DATABASE_URL` and
`TEST_CHROMA_BASE_URL` to disposable services, then run
`uv run pytest tests/integration tests/retrieval -m integration`. PostgreSQL fixtures create and
remove uniquely named test databases; the test principal needs database-creation privileges.
Chroma tests create and remove uniquely named collections. Without these variables, service checks skip.
Normal tests inject probes and need no services or inference calls.

## Mock data and retrieval foundation

```powershell
uv run python scripts/generate_mock_data.py --count 20 --seed 42 --output data/demo
```

The default count is 200 with a 140/40/10/10 category allocation. The generator emits
`incidents.jsonl`, canonical `approved_articles.jsonl`, evaluation-only `ground_truth.jsonl`,
and a hash manifest. Repeating identical arguments is safe; a different seed/count requires
a different output directory. Calibration and test families use distinct cause/remedy templates.
This small synthetic fixture does not establish model quality or replace curator adjudication.

`MockJiraAdapter.from_file` accepts only the incident file. It checks scope on every read,
binds pagination to the actor/filters/dataset revision, supports resolution dates and statuses,
and preserves oversized histories with an incomplete marker. Oracle sidecars are rejected.
Database seeding and the complete demo workflows remain unfinished.

`SearchKnownErrors` combines injected exact, lexical, and optional vector ports. PostgreSQL
revalidates approved current versions and scope before ranking is persisted. Raw scores and
cosine distances remain separate from reciprocal-rank fusion scores. Missing/failed paths,
stale vectors, and a mismatched active embedding model cannot establish a complete no-match.
These application interfaces are not yet exposed through search HTTP routes or UI pages.

The Chroma adapter uses bounded v2 HTTP requests because the installed Python HTTP client
disables timeouts. It stores vectors and provenance IDs, without article text or query text.
Ollama embedding calls validate the configured digest before/after embedding and explicitly
disable truncation. Document embedding and vector upsert require SQL-verified approved chunks.
Only Chroma was exercised live; Ollama protocol tests use controlled HTTP responses.
